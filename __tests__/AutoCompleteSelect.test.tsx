...
describe('AutoCompleteSelect', () => {
  ...
  it('renders without errors', () => {
    render(<AutoCompleteSelect id='autocomplete' options={options} placeholder={placeholder} />);
  });
  ...
  it('renders options correctly', async () => {
    render(<AutoCompleteSelect id='autocomplete' options={options} placeholder={placeholder} />);
    const selectInput = screen.getByPlaceholderText(placeholder);
    await act(async () => {
  ...
  it('selects default value correctly', () => {
    const defaultValue = { label: 'Amadeus', value: 1984 };
    ...
    const selectInput = screen.getByPlaceholderText(placeholder) as HTMLInputElement;
    expect(selectInput.value).toBe(defaultValue.label);
  });

  it('selects default multiple value  correctly', () => {
    const defaultValue = [
      { label: 'City Lights', value: 1931 },
      { label: 'Amadeus', value: 1984 }
    ];
  ...
  it('selects one option correctly', () => {
    render(<AutoCompleteSelect id='autocomplete' options={options} placeholder={placeholder} />);
    const autoCompleteInput = screen.getByPlaceholderText(placeholder) as HTMLInputElement;

    fireEvent.mouseDown(autoCompleteInput);

    const option1 = screen.queryAllByText('City Lights')[0];
    fireEvent.click(option1);
    expect(autoCompleteInput.value).toBe('City Lights');
  ...
  it('selects multiple options correctly', () => {
    ...
    const option2 = screen.queryAllByText('Amadeus')[0];
